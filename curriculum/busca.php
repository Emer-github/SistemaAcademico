...
/*Condicion para hacer la busqueda dentro de la paginación*/
  if($action == 'ajax'){
    ...
    //Arreglo de columnas donde se realizara la busqueda. Deacuerdo a las columnas
     $aColumns = array('curriculum.idcurriculum','curriculum.ahnio', 'docente.nombres', 'docente.apellidos');//Columnas de busqueda
     $Table = "curriculum"; //Nombre tabla de BD
...
//contamos los valores encontrados y realiza consulta.
    $count_query = "SELECT curriculum.*, docente.nombres as docenteNombre, docente.apellidos as docenteApellido FROM $Table inner join docente on curriculum.iddocente=docente.iddocente $Where";
...
// Consulta de la paginación
      $sql = "SELECT curriculum.*, docente.nombres as docenteNombre, docente.apellidos as docenteApellido FROM $Table inner join docente on curriculum.iddocente=docente.iddocente $Where ORDER BY curriculum.idcurriculum desc LIMIT $offset,$per_page";
...
foreach($model as $row){ //Mostrar los datos encontrados en la BD $model, $row tomados lineas atras.
  ?>
      ...
      <td class="success"><?php echo $row['idcurriculum'];?></td>
      <td><?php echo $row['ahnio'];?></td>
      <td><?php echo $row['docenteNombre'];?></td>
      <td><?php echo $row['docenteApellido'];?></td>
      <?php 
      echo "<td align='center' width='10%' class='active'><a href='".$row['curriculum']."' class='btn btn-primary btn-xs;'><i style='color:white;' class='fa fa-download'></i></a></td>";
     ?>
      </tr>
      <!-- envio de datos-->
      <input type="hidden" value="<?php echo $row['iddocente'];?>" id="iddocente<?php echo $row['idcurriculum'];?>">    
<?php
