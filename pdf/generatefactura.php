...
//consult para seleccionar todos los datos de la venta
 ...
 $command = "SELECT detalleventa.cantidad,producto.nombre as nombrepr,producto.descripcion as descri,detalleventa.precioactual,detalleventa.subtotal FROM detalleventa inner join producto
  on detalleventa.idproducto = producto.idproducto WHERE idventa = '$idtipo'";
...
while($rowss = $resultado->fetch())
{
  ...
  $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    ...
    venta.idusuario= usuario.idusuario inner join cliente on venta.idcliente = cliente.idcliente where idventa = $idtipo";
    $q = $conn->prepare($sql);
    $q->execute(array($id));
...
$que ="SELECT sum(cantidad) as cant,sum(subtotal) as totaldinero FROM detalleventa  WHERE idventa = $idtipo";
 $qu = $conn->prepare($que);
    $qu->execute(array($id));
