<?php
  if(!empty($_GET['idgrado'])){
    ...
    $sql = "SELECT matricula.*, estudiante.nombres as nombreEstudiante, estudiante.apellidos as apellidoEstudiante, estado.nombre as estado, notas.idnotas as idnotas, notas.primerTrimestre as n1, notas.segundoTrimestre as n2, notas.tercerTrimestre as n3, notas.notaFinal as nF FROM matricula inner join estado on matricula.idestado=estado.idestado inner join estudiante on matricula.nie=estudiante.nie inner join notas on matricula.idmatricula=notas.idmatricula WHERE matricula.idgradoDocente='$grado' and notas.idasignatura='$asignatura' ORDER BY estudiante.apellidos ASC";
