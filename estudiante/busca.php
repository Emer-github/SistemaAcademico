...
/*Condicion para hacer la busqueda dentro de la paginación*/
  if($action == 'ajax'){
    ...
    //Arreglo de columnas donde se realizara la busqueda. Deacuerdo a las columnas
     $aColumns = array('estudiante.nie', 'estudiante.nombres','estudiante.apellidos','estudiante.genero', 'padre.nombres','madre.nombres','responsable.nombres');//Columnas de busqueda
...
//contamos los valores encontrados y realiza consulta.
      $count_query = "SELECT estudiante.*, padre.nombres as padreNombre, madre.nombres as madreNombre, responsable.nombres as responsableNombre FROM $Table inner join padre on estudiante.duiPadre=padre.dui inner join madre on estudiante.duiMadre = madre.dui inner join responsable on estudiante.duiResponsable = responsable.dui $Where and estudiante.idestado='1'";
...
// Consulta de la paginación
      $sql = "SELECT estudiante.*, padre.nombres as padreNombre, madre.nombres as madreNombre, responsable.nombres as responsableNombre FROM $Table inner join padre on estudiante.duiPadre=padre.dui inner join madre on estudiante.duiMadre = madre.dui inner join responsable on estudiante.duiResponsable = responsable.dui $Where and estudiante.idestado='1' ORDER BY estudiante.nie desc LIMIT $offset,$per_page";
...
<!-- Encabezada con efecto success-->
  ...
  <!-- style="text-align: center; Centrara el contenido de la columna principal ☺☺☺☺ -->
    ...
    <th style="text-align: center;">Madre</th>
...
foreach($model as $row){ //Mostrar los datos encontrados en la BD $model, $row tomados lineas atras.
  ?>
      ...
      <td><?php echo $row['padreNombre'];?></td>
      <td><?php echo $row['madreNombre'];?></td>
    ...
    <?php 
     ...
     ?>
      ...
      <input type="hidden" value="<?php echo $row['nacionalidad'];?>" id="nacionalidad<?php echo $row['nie'];?>">
      <input type="hidden" value="<?php echo $row['telefono'];?>" id="telefono<?php echo $row['nie'];?>"> 
      <input type="hidden" value="<?php echo $row['zona'];?>" id="zona<?php echo $row['nie'];?>">
      <input type="hidden" value="<?php echo $row['direccion'];?>" id="direccion<?php echo $row['nie'];?>">
      <input type="hidden" value="<?php echo $row['enfermedad'];?>" id="enfermedad<?php echo $row['nie'];?>">
      <input type="hidden" value="<?php echo $row['alergia'];?>" id="alergia<?php echo $row['nie'];?>">
     <?php
