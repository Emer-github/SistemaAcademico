...
/*Condicion para hacer la busqueda dentro de la paginación*/
  if($action == 'ajax'){
    ...
    //Arreglo de columnas donde se realizara la busqueda. Deacuerdo a las columnas
     $aColumns = array('idcomentario','comentario','fecha');//Columnas de busqueda
     $Table = "comentario"; //Nombre tabla de BD
...
$count_query = "SELECT *, DATE_FORMAT(fecha, '%d/%m/%Y') AS miFecha FROM $Table $Where";
// Conexión con pdo
...
$sql = "SELECT *, DATE_FORMAT(fecha, '%d/%m/%Y') AS miFecha FROM $Table $Where ORDER BY idcomentario desc LIMIT $offset,$per_page";
...
foreach($model as $row){ //Mostrar los datos encontrados en la BD $model, $row tomados lineas atras.
  ?>
    <tr style="text-align: center;">
      <td class="success"><?php echo $row['idcomentario'];?></td>
      <td><?php echo $row['miFecha'];?></td>
      <td><?php echo $row['comentario'];?></td>
      <td class="active">
        <span class="tooltips" data-placement="left" data-original-title="Denegar solicitud">   
          <a class='btn btn-danger btn-xs;' style='cursor:pointer;' data-toggle='modal' data-target='#modal_rojo' onclick="eliminar(<?php echo ($row['idcomentario']); ?>)" ><i style="color:white;font-size:20px;" class='fa fa-trash-o'></i></a>     
        </span>
      </td>
    </tr>
      <!-- envio de datos-->
      <input type="hidden" value="<?php echo $row['comentario'];?>" id="comentario<?php echo $row['idcomentario'];?>">
     <?php
