...
//contamos los valores encontrados y realiza consulta.
    if ($idusu==1) {
      $count_query = "SELECT usuario.*,tipousuario.nombre as tiponombre, docente.nombres as docenteNombre FROM $Table inner join tipousuario on usuario.idtipoUsuario=tipousuario.idtipoUsuario inner join docente on usuario.iddocente = docente.iddocente $Where and usuario.estado='Inactivo'";
    }elseif ($idusu!=1) {
      $count_query = "SELECT usuario.*,tipousuario.nombre as tiponombre, docente.nombres as docenteNombre FROM $Table inner join tipousuario on usuario.idtipoUsuario=tipousuario.idtipoUsuario inner join docente on usuario.iddocente = docente.iddocente $Where and usuario.estado='Inactivo'";
    }
...
// Consulta de la paginación
    if ($idusu==1) {
      $sql = "SELECT usuario.*,tipousuario.nombre as tiponombre, docente.nombres as docenteNombre FROM $Table inner join tipousuario on usuario.idtipoUsuario=tipousuario.idtipoUsuario inner join docente on usuario.iddocente = docente.iddocente $Where and usuario.estado='Inactivo' ORDER BY usuario.idusuario desc LIMIT $offset,$per_page";
    }elseif ($idusu!=1) {
      $sql = "SELECT usuario.*,tipousuario.nombre as tiponombre, docente.nombres as docenteNombre FROM $Table inner join tipousuario on usuario.idtipoUsuario=tipousuario.idtipoUsuario inner join docente on usuario.iddocente = docente.iddocente $Where and usuario.estado='Inactivo' ORDER BY usuario.idusuario desc LIMIT $offset,$per_page";
    }
