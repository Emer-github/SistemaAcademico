...
class padre{
	...
	function guardar($dui, $nombres, $apellidos, $nacimiento, $profesion, $telefono, $direccion){
		try{
	 		...
	 		$stmt = $conn->prepare('INSERT INTO padre(dui,nombres,apellidos,nacimiento,profesion,telefono,direccion) VALUES(:a,:b,:c,:d,:e,:f,:g)');	
	 		$stmt->bindParam(':a',$a);
	 		$stmt->bindParam(':b',$b);
	 		$stmt->bindParam(':c',$c);
	 		$stmt->bindParam(':d',$d);
	...
	function editar($dui, $nombres, $apellidos, $nacimiento, $profesion, $telefono, $direccion){
		...
		$sql = "UPDATE padre SET nombres = '$nombres', apellidos = '$apellidos', nacimiento = '$nacimiento', profesion = '$profesion', telefono = '$telefono', direccion = '$direccion' where dui = '$dui'";
