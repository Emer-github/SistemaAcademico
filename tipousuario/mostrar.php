...
foreach($model as $row){
      ...
      $esta = $row['estado'];
        ...
        if ($nom =='Administrador'){
          ...
          echo "<td align='center' width='10%' class='active'><a href='editarPermisos.php?idTipo=".$row['idtipoUsuario']."' class='btn btn-warning'><i style='color:white;' class='fa fa-lock'></i></a></td>";  
          //echo "<button class='btn btn-primary btn-xs;' data-toggle='modal' data-target='#modal_azul' onclick='permiso(".$row['idtipoUsuario'].");''><i style='color:white;' class='fa fa-lock'></i></button>"; 
        }
      ?>
