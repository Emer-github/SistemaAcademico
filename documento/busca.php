...
/*Condicion para hacer la busqueda dentro de la paginación*/
  if($action == 'ajax'){
    ...
    //Arreglo de columnas donde se realizara la busqueda. Deacuerdo a las columnas
     $aColumns = array('documento.iddocumento','documento.ahnio', 'documento.tipo', 'documento.descripcion');//Columnas de busqueda
     $Table = "documento"; //Nombre tabla de BD
...
//contamos los valores encontrados y realiza consulta.
    $count_query = "SELECT documento.*, usuario.usuario as usuario FROM $Table inner join usuario on documento.idusuario=usuario.idusuario $Where";
...
// Consulta de la paginación
      $sql = "SELECT documento.*, usuario.usuario as usuario FROM $Table inner join usuario on documento.idusuario=usuario.idusuario $Where ORDER BY documento.iddocumento desc LIMIT $offset,$per_page";
...
foreach($model as $row){ //Mostrar los datos encontrados en la BD $model, $row tomados lineas atras.
  ?>
      ...
      <td class="success"><?php echo $row['iddocumento'];?></td>
      <td><?php echo $row['ahnio'];?></td>
      <td><?php echo $row['tipo'];?></td>
      <td><?php echo $row['descripcion'];?></td>
      <?php 
      echo "<td align='center' width='10%' class='active'><a href='".$row['documento']."' class='btn btn-primary btn-xs;'><i style='color:white;' class='fa fa-download'></i></a></td>";
     ?>
      </tr>
      <!-- envio de datos-->
      <input type="hidden" value="<?php echo $row['tipo'];?>" id="tipo<?php echo $row['iddocumento'];?>">
      <input type="hidden" value="<?php echo $row['descripcion'];?>" id="descripcion<?php echo $row['iddocumento'];?>">    
<?php
