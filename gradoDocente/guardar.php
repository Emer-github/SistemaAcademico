<?php
  if(!empty($_POST)){
      ...
      $save = $send->guardar($año,$turno,$tipo,$guia,$grado);
          ...
          $consult = $conn->prepare("SELECT max(idgradoDocente) as idg FROM grado_docente");
          $consult->execute();
          $data = $consult->fetch(PDO::FETCH_ASSOC);
          $idgradoDocente = $data['idg'];
          if (count($asignatura)==count($docente)) {
            for ($i=0; $i < count($asignatura); $i++) { 
              $send->guardarDetalle($idgradoDocente,$docente[$i],$asignatura[$i]);
            }
          }else{
            echo "algo salio mal";
          }
