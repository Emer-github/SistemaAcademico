...
class gradoDocente{
	...
	function guardar($año,$turno,$tipo,$guia,$idgrado){
		try{
	 		...
	 		$stmt = $conn->prepare('INSERT INTO grado_docente(ahnio,turno,tipo,guia,idgrado) VALUES(:a,:b,:c,:d,:e)');	
	 		$stmt->bindParam(':a',$a);
	 		$stmt->bindParam(':b',$b);
	 		$stmt->bindParam(':c',$c);
	 		$stmt->bindParam(':d',$d);
	...
	function editar($id,$año,$turno,$tipo,$guia,$idgrado){
		...
		$sql = "UPDATE grado_docente SET ahnio = '$año', turno = '$turno', tipo = '$tipo', guia = '$guia', idgrado = '$idgrado' where idgradoDocente = '$id'";
